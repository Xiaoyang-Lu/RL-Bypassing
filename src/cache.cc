...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
	    ...
	    action = llc_update_bypass_state_decision(fill_cpu, set, 0, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, 0, current_core_cycle[fill_cpu]);
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, current_core_cycle[fill_cpu]);
        }
        else
	...
	if ((cache_type == IS_LLC) && (action == 1)) { // this is a bypass that does not fill the LLC
            ...
            if (cache_type == IS_LLC) {
                //llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, pure_miss_tag);
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) {
		action = llc_update_bypass_state_decision(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1, 0, current_core_cycle[writeback_cpu]);
		  ...
		  if(cache_type == IS_LLC)
		  ...
		  else
		    {
		      ...
		      lower_level->add_rq(&WQ.entry[index]);
                ...
                if (cache_type == IS_LLC) {
                    way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, current_core_cycle[writeback_cpu]);
                }
                else
			...
			cpu = 0;
                    ...
                    // update replacement policy
                    if (cache_type == IS_LLC) {
        	       action = llc_update_bypass_state_decision(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0, 0, current_core_cycle[writeback_cpu]); 
      ...
      if (read_cpu == NUM_CPUS)
      {
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
		    action = llc_update_bypass_state_decision(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, 0, current_core_cycle[read_cpu]);
		    llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, 0);
      ...
      if (prefetch_cpu == NUM_CPUS)
      {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {            
            ...
            if (way >= 0) { // prefetch hit
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
 		    action = llc_update_bypass_state_decision(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, 0, current_core_cycle[prefetch_cpu]);
                    llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, 0);
